...
#%% 3.18 Lectura de todos los arboles

def leer_arboles(nombre_archivo):
    
    with open(nombre_archivo, 'rt', encoding='utf8') as f:
        types = [float, float, int, float, int, float, int, str, str, str,str,str,str,str,str, float, float]
        arboles = csv.reader(f)
        headers = next(arboles)
        arboles_converted = [[func(val) for func, val in zip(types, arbol)] for arbol in arboles]
        arboleda = [{ncolumna : arbol[n] for n, ncolumna in enumerate(headers)} for arbol in arboles_converted]
    return arboleda    

arboleda = leer_arboles('Data/data_verde.csv')

#3.19: Lista de altos de Jacarandá
altura_jacarandas = [arbol['altura_tot'] for arbol in arboleda if arbol['nombre_com'] =='Jacarandá']

#3.20: Lista de altos y diámetros de Jacaranda
altura_diam_jacarandas = [(arbol['altura_tot'],arbol['diametro']) for arbol in arboleda if arbol['nombre_com'] =='Jacarandá']

#3.21: Diccionario con medidas
...
species = ['Eucalipto', 'Palo borracho rosado', 'Jacarandá']

diccionario_medidas = medidas_de_especies(species, arboleda)
